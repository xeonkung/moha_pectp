...
public:
    ...
    void localSearch(int maxSteps, double LS_limit = 999999, double prob1 = 1.0, double prob2 = 1.0, double prob3 = 0.0, bool evSort = false); //apply local search with the given parameters
...
private:
    ...
    vector<int> shuffle(vector<int> source);
...
public:
    ...
    CompareEvent(Solution *a) {
        s = a;
    }

    bool operator()(int a, int b) {
        if (!s->feasible) {
            return s->eventHcv(a) > s->eventHcv(b);
        } else {
            return s->eventScv(a) > s->eventScv(b);
        }
    };
};
